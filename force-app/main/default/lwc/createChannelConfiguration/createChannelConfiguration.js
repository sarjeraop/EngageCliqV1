...
import DUPLICATEERRORMSG from "@salesforce/label/c.Error_msg_for_Duplicate_Cannel_Configuration";

const actions = [
  { label: 'Edit', name: 'Edit' },
];

export default class CreateChannelConfiguration extends LightningElement {
...
//@wire(getRecord, { recordId: "$recordId", fields: "$filedSet"})
  ...
  getChannelConfigMetadata(wireResult) {
    ...
    handelCancle(){
      ...
      this.recChannelConfig.API_key__c = '';
      this.recChannelConfig.Business_ID__c = '';
      this.recChannelConfig.PhoneNo_Id__c = '';
      this.recChannelConfig.Endpoint_URL__c = '';
    }
    ...
    handelNamechange(event){  
      ...
      {
        if(this.data[i].DeveloperName.toLowerCase() === event.target.value.toLowerCase())
        {
          this.enableSave = true;
          this.showNotification('Error',DUPLICATEERRORMSG,'Error');
          continue;
